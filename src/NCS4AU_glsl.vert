...
uniform vec3  uNoiseFlowSpeed;      // ノイズ変異速度 (x, y, z)
uniform ivec3 uFlags;               // (isReactive: bool, isSpherical: bool, applyNoise: bool)
uniform float uTime;                // フレーム数
uniform float uAudioStrength;       // 音声強度
...
void main() {
	...
	bool isReactive  = uFlags.x == 1;
	bool isSpherical = uFlags.y == 1;
	bool applyNoise  = uFlags.z == 1;

	if (applyNoise) {
		float freq = uTrackParams.z * 0.001;
		float flowX = uTime * -(uNoiseFlowSpeed.x * 0.1);
		float flowY = uTime *  (uNoiseFlowSpeed.y * 0.1);
		float flowZ = uTrackParams.w;
		const float dimOff = 100.0;
		float amp = uTrackParams.y;
		if (isReactive) amp += uAudioStrength * uReactionMultipliers.y;
	...
	if (isSpherical && uProjectionFactor > 0.0) {
		float radius = uTrackParams.x * 0.5;
		if (isReactive) radius += uAudioStrength * (uReactionMultipliers.x * 0.1);
		float len = length(pos);
		vec3 projected = pos * (radius / max(len, 1e-6));
		projected.z = abs(projected.z);
		pos = mix(pos, projected, uProjectionFactor);
	}
